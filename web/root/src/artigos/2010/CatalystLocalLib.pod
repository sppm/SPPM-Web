...
do seu arquivo .bashrc, e então a cada login elas serão automaticamente
exportadas.
...
Ele foi instalado dentro de uma árvore de diretórios criada no index.t do usuário
...
criado no diretório index.t do usuário corrente. Ele possui somente duas linhas
conforme abaixo:
...
Onde /index.t/blabos é o diretório index.t do usuário, no meu caso, blabos; e
...
no mesmo diretório que foi configurado com local::lib, /index.t/blabos/perl5 no
meu caso.
...
B<Catalyst::Plugin::únicode> - Suporte transparente a únicode (UTF8).
