...
A ideia principal é poder oferecer um design para promover a portabilidade e
...
O parâmetro "ts" diz qual o "tempo" do dado informado, e ele sempre é referenciado
pelo nome "ts" (timeseries) e o valor é baseado no "epoch time", se este parâmetro
...
O único parâmetro que não vem com valor, é o "foo", por é justamente o valor dele
